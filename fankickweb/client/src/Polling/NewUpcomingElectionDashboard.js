...
import { Col, Table, Icon, Form, Modal, Input, Upload, Pagination, Switch, Select, Tooltip, message, Button } from 'antd';
...
class NewUpcomingElectionDashboard extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            visible: false,
    ...
    getContests = () => {
        ...
        axios.get(process.env.REACT_APP_API_HOST + '/rest/optionPollsWeb?' + 'type' + '=' + 'upComing' + '&' + "offset=" + offset + '&' + "limit=" + limit, {
            headers: {
                "x-access-token": sessionStorage.token,
            },
        })
            .then(function (response) {
                console.log("reposne in dash upcoming44", response.data.data);
    ...
    showModal = (e) => {
        ...
        axios.get(process.env.REACT_APP_API_HOST + '/rest/optionPollingById?pollId=' + id, {
            ...
            .then(function (response) {
                console.log("data for show modal upcoming view", response.data.data)
    ...
    handleOk01 = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            var data = {
                ...
                "pollText": this.state.declarewinnersid.pollTitle,
            ...
            fetch(request)
                ...
                .then(function (response) {
                    ...
                    if (response.statusMessage === "Success") {
                        ...
                        message.error(`Unable to create Declare Winners.`, 5);
                    }
                })
        }
    ...
    showModaldelete = (e) => {
        ...
        const url = process.env.REACT_APP_API_HOST + '/rest/optionPollDelete';
        var request = new Request(url, {
            method: 'DELETE',
            body: JSON.stringify(data),
            headers: {
        ...
        fetch(request)
            ...
            .then(function (response) {
                if (response.statusMessage === "Success") {
                    ...
                    message.success("poll is deleted successfully");
                }
                else {
                    message.error("Unable to delete the poll.");
                }
            })
    ...
    pageChange = (page, pageSize) => {
        ...
        axios.get(process.env.REACT_APP_API_HOST + '/rest/optionPollsWeb?' + 'type' + '=' + 'upComing' + '&' + "offset=" + newoffset + '&' + "limit=" + newlimit, {
            headers: {
                "x-access-token": sessionStorage.token,
            },
        })
            ...
            .then(function (response) {
                console.log("Fun2Win responce data on upComing", response.data.data)
    ...
    render() {
        ...
        console.log("viewdatain upcoming", this.state)
        ...
        completeddata === undefined || completeddata.length === 0 ? null : completeddata.map((item, index) => {
            data.push({
                ...
                Action: <div id={item.pollId}> <span className="PolliunderView">
                    <a id={item.pollId} onClick={this.showModal.bind(this, item.pollId)}><Icon type="eye-o" /></a>
                </span>
                    <span className="PolliunderView">
                        <Link to={`/Polling/CreateNewElection/${item.pollId}`}>  <Icon type="edit" /></Link>
                    </span>
                    <span className="PolliunderView">
                        <a id={item.pollId} onClick={this.showModaldelete.bind(this, item.pollId)}><Icon type="delete" /></a>
                    </span></div>
            })
                        ...
                        <Form>
                                    ...
                                    <li>
                                        <FormItem className={classnames('selectwinningtype', { error: !!this.state.errors.selectwinningtype })}>
                                            ...
                                            <Select value={this.state.selectwinningtype || undefined} onChange={this.onselectPollType}
                                                style={{ width: 120 }} placeholder="Please choose one"
                                                >
                                    ...
                                    {this.state.selectwinningtype === "result" ?
                                        <FormItem className={classnames('resulttypevalue', { error: !!this.state.errors.resulttypevalue })}>
                                            ...
                                            <Select
                                                ...
                                                onChange={this.resultChange} style={{ width: 120 }}
                                                >
                                                <Option value={this.state.declareddatabyid.options[0].option}>{this.state.declareddatabyid.options[0].option}</Option>
                                                <Option value={this.state.declareddatabyid.options[1].option}> {this.state.declareddatabyid.options[1].option}</Option>
                            ...
                            <Col span={24}>
                                <FormItem className={classnames('coupontype', { error: !!this.state.errors.coupontype })}>
                                    ...
                                    <Select value={this.state.coupontype} onChange={this.onChnagecoupon} style={{ width: 120 }}
                                        getPopupContainer={triggerNode => triggerNode.parentNode}>
                                ...
                                <Col span={24}>
                                    ...
                                    <Col span={13}>
                                        <FormItem className={classnames('mrchaname', { error: !!this.state.errors.mrchaname })}>
                                            ...
                                            <Input placeholder="Basic usage" name="mrchaname" value={this.state.mrchaname} onChange={this.onChangeName} />
                                        ...
                                        <FormItem className={classnames('coponscost', { error: !!this.state.errors.coponscost })}>
                                            ...
                                            <Input placeholder="Basic usage" name="coponscost" value={this.state.coponscost} onChange={this.onChangeName} />
...
export default NewUpcomingElectionDashboard;
/* eslint-disable */