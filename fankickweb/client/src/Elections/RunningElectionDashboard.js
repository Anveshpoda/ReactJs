...
import { Col, Card, Dropdown, Icon, Menu, Modal } from 'antd';
import $ from 'jquery';
...
class RunningElectionDashboard extends React.Component {
    static propTypes = {
    ...
    componentDidMount() {
        //  console.log("runnning called srvice")
        this.getContests();
    }
    getContests = () => {
        axios.get(process.env.REACT_APP_API_HOST + '/rest/optionPollsWeb', {
            ...
            .then(function (response) {
                ...
                var hhh = response.data.data.reverse()
                var contentPack = [];
                for (let key in hhh) {
                    var keyObj = hhh[key];
                    if (new Date().getTime() >= new Date(keyObj.pollStartDateTime).getTime() &&
                        new Date().getTime() <= new Date(keyObj.pollEndDateTime).getTime()) {
                    ...
                    <div>
                        <Col xs={12} sm={12} xl={4} lg={6} md={6}>
                            <Card>
                                <div className="image">
                                    <figure>
                                        <img className="Boximage challengesmaincardImage" src={contest.pollImageUrl != '' ? contest.pollImageUrl : amitabh1} alt="Contest Cover" width='100%' height='175px' />
                                ...
                                <div className="cardContent">
                                    <ul>
                                        ...
                                        <li className="ElectionPollSubtext1" title={contest.pollId}>
                                            <h5 className="PolligUniquenumber">UniqueNumber:<span className="PollStartDate">{contest.pollId}</span></h5>
                                            </li>*/}
                                    </ul>
                                </div>
                                <div className="cardFooter">
                                    <ul className="list-inline">
                                        <li style={{ color: 'orange' }}><b>Running</b></li>
...
export default RunningElectionDashboard;
/* eslint-disable */