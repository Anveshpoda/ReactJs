...
import { Menu, Icon, Row, Col, Card, Button, Dropdown, DatePicker, Pagination, message, Select, Modal, Spin, Alert, Form } from 'antd';
...
class ClosedList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      categories: [],
  ...
  getContests = (id) => {
    ...
    axios.get('/closed/' + this.state.pagenumber + '/' + id, {
      headers: {
      ...
      .then(function (response) {
        _this.setState({ contests: response.data.data, loading: false, numOfPages: response.data.numofPages });
  ...
  fetchRequest = (subid) => {
    ...
    axios.get('/pack-by-subcategory/Closed/' + this.state.pagenumber + '/' + this.state.freelancerId, {
      params: {
        subCatId: subid
      },
      headers: {
      ...
      //   if (contest.isDeleted === true) {
      //         console.log("contest",contest.isDeleted)
      //     render.push(contest);
      //   }
      // }
  ...
  onCelebrityChange(value) {
    ...
    axios.get('/pack-by-celebrityname/Closed/' + this.state.pagenumber + '/' + this.state.freelancerId, {
      params: {
        celName: celname
      },
      headers: {
  ...
  pageChange = (page, pageSize) => {
    ...
    if (this.state.subCategoryId != '') {
      var url = '/pack-by-subcategory/Closed/' + page + '/' + this.state.freelancerId + '?subCatId=' + this.state.subCategoryId + '';
    }
    else if (this.state.celebrityName != '') {
      var url = '/pack-by-celebrityname/Closed/' + page + '/' + this.state.freelancerId + '?celName=' + this.state.celName + '';
    }
    else {
      var url = '/closed/' + page + '/' + this.state.freelancerId
    }
    axios.get(url, {
      headers: {
        "x-access-token": sessionStorage.getItem('token'),
          ...
          <Card>
            ...
            <div className="cardFooter">
              <ul className="list-inline">
                <li><b className="fun2winmainfoot">Closed</b></li>
                <li className="floatRight">
                  <Dropdown getPopupContainer={triggerNode => triggerNode.parentNode} overlay={<Menu>
                    {(closed.reschedule === true) ?
                    <Menu.Item key="0">
                      <a href="#" ><span id={contest._id} onClick={this.ApproveModal.bind(this)}>Reschedule</span></a>
                    </Menu.Item>:null}
                    {/* <Menu.Item key="1">
            ...
            <Col span={24} className='challengesmenu'>
              <div className="fun2winSubMenu">
                ...
                <Col span={4} className='fun2winSelect fun2winSelectcategry2'> <Select
                  ...
                  value={this.state.celName || undefined}
                  filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                  getPopupContainer={triggerNode => triggerNode.parentNode} placeholder="Select Celebrity" style={{ width: '100%' }} onChange={this.onCelebrityChange}>
                  {/* {this.state.subCategories.length < 0? <h1>No SubCategories available</h1>:{mapSubCategories}}  */}
                  {mapCelebrities}
                </Select></Col>
                <Col span={9} className="fun2winmainheader">
