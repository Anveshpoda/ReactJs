...
class Questions extends React.Component {
    ...
    componentWillReceiveProps(newProps) {
        console.log(newProps, 'Main Props')
        let a = [];
        // console.log(newProps.subQuestions, 'subQuestions')
        if (newProps.subQuestions !== undefined) {
            if (newProps.subQuestions.length > 0) {
                a = newProps.subQuestions.map((element, index) => {
                    this.setState({
                        [newProps.questionId + '@SubQuestion' + (index + 1)]: element.name,
                        newTabIndex: newProps.subQuestions.length > this.state.newTabIndex ? newProps.subQuestions.length : this.state.newTabIndex
                    })
                    if (element.optionMediaType === "image" && newProps.mediaUrl) {
                        let textArray = [];
                        let answer = element.correctAnswer;
                        element.options.map((record, recordIndex) => {
                            textArray.push('SubQuestion' + (index + 1) + '@OptionDragNDrop-' + (recordIndex + 1))
                            if (answer == (recordIndex + 1)) {
                                this.setState({
                                    ['SubQuestion' + (index + 1) + '@OptionDragNDrop-' + (recordIndex + 1)]: record.option,
                                    ['CB&SubQuestion' + (index + 1) + '@OptionDragNDrop-' + (recordIndex + 1)]: true
                                ...
                                this.setState({
                                    ['SubQuestion' + (index + 1) + '@OptionDragNDrop-' + (recordIndex + 1)]: record.option,
                                    ['CB&SubQuestion' + (index + 1) + '@OptionDragNDrop-' + (recordIndex + 1)]: false,
                                })

                            }
                        })
                        if (textArray.length === element.options.length) {
                            this.setState({
                                ['optionDragNDropArraySubQuestion' + (index + 1)]: textArray,
                                ['optionTextArraySubQuestion' + (index + 1)]: [],
                                ['optionImageUploadArraySubQuestion' + (index + 1)]: [],
                                ['optionMediaTypeSubQuestion' + (index + 1)]: 'Drag&Drop',
                                ['optionAnswerTypeSubQuestion' + (index + 1)]: element.answerType,
                                "combinationImage": newProps.mediaUrl
                            })
                        }
                    }
                    else if (element.optionMediaType === "text") {
                        let textArray = [];
                        let answer = element.correctAnswer;
                        element.options.map((record, recordIndex) => {
                            textArray.push('SubQuestion' + (index + 1) + '@OptionText-' + (recordIndex + 1))
                            if (answer == (recordIndex + 1)) {
                                this.setState({
                                    ['SubQuestion' + (index + 1) + '@OptionText-' + (recordIndex + 1)]: record.option,
                                    ['CB&SubQuestion' + (index + 1) + '@OptionText-' + (recordIndex + 1)]: true
                                })
                            }
                            else {
                                this.setState({
                                    ['SubQuestion' + (index + 1) + '@OptionText-' + (recordIndex + 1)]: record.option,
                                    ['CB&SubQuestion' + (index + 1) + '@OptionText-' + (recordIndex + 1)]: false
                                })
                        ...
                        if (textArray.length === element.options.length) {
                            this.setState({
                                ['optionTextArraySubQuestion' + (index + 1)]: textArray,
                                ['optionImageUploadArraySubQuestion' + (index + 1)]: [],
                                ['optionDragNDropArraySubQuestion' + (index + 1)]: [],
                                ['optionMediaTypeSubQuestion' + (index + 1)]: 'text',
                                ['optionAnswerTypeSubQuestion' + (index + 1)]: element.answerType
                            })
                        }
                    }
                    else if (element.optionMediaType === "image") {
                        let imageArray = [];
                        let answer = element.correctAnswer;
                        element.options.map((record, recordIndex) => {
                            imageArray.push('SubQuestion' + (index + 1) + '@OptionImage-' + (recordIndex + 1))
                            if (answer == (recordIndex + 1)) {
                                this.setState({
                                    ['SubQuestion' + (index + 1) + '@OptionImage-' + (recordIndex + 1)]: record.option,
                                    ['CB&SubQuestion' + (index + 1) + '@OptionImage-' + (recordIndex + 1)]: true
                                })
                            }
                            else {
                                this.setState({
                                    ['SubQuestion' + (index + 1) + '@OptionImage-' + (recordIndex + 1)]: record.option,
                                    ['CB&SubQuestion' + (index + 1) + '@OptionImage-' + (recordIndex + 1)]: false
                                })
                            }
                        })
                        if (imageArray.length === element.options.length) {
                            this.setState({
                                ['optionImageUploadArraySubQuestion' + (index + 1)]: imageArray,
                                ['optionTextArraySubQuestion' + (index + 1)]: [],
                                ['optionDragNDropArraySubQuestion' + (index + 1)]: [],
                                ['optionMediaTypeSubQuestion' + (index + 1)]: 'image',
                                ['optionAnswerTypeSubQuestion' + (index + 1)]: element.answerType
                            })
                        }
                    }
        ...
        this.setState({
            ...
            panes: a.length > 0 ? a : this.state.panes
    ...
    add = () => {
        ...
        const activeKey = `SubQuestion${++this.state.newTabIndex}`;
    ...
    handleAnswerTypeChange = (key, paneNumber, e) => {
        ...
        if (e === "text") {
            console.log(paneNumber, '12112122', 'CB&' + paneNumber + '@OptionText-1')
            this.setState({
                [key]: e,
    ...
    savePackQuestion = (key, e) => {
        ...
        if (questions.length > 0 && Object.keys(this.state.errors).length === 0 && this.state.panes.length === questions.length) {
            this.props.saveSubQuestionPacks(questions, this.state.queueId)
        }
        if (Object.keys(this.state.errors).length > 0) {
            message.success(`You have unsaved items.`)
        }

    }

    render() {
        // console.log('Render Called..', this.state.panes)
        const props = {
            name: 'file',
                    ...
                    <div className="Redfun2winsubques">
                        <ul className="list-inline">
                            ...
                            <li className="ant-col-6">
                                {this.state.category != "text" ? <Button className="RightAlignButn AddmoreBtn" onClick={this.add.bind(this)}>Add Sub Question</Button> : null} </li>
                        </ul>
                                                ...
                                                <Col span={24}>
                                                    <Col span={13}>
                                                        <FormItem label="Type Your Question OR Brief(Optional)">
                                                            <TextArea rows={3} placeholder="I Enter here" name={this.state.queueId + "@" + pane.key} onChange={this.handleInputChange} value={this.state[this.state.queueId + "@" + pane.key]} />
                                                    ...
                                                    <Col span={5}>
                                                        <FormItem label="Add Answers">
                                                            <Select getPopupContainer={triggerNode => triggerNode.parentNode}
                                                                ...
                                                                value={this.state['optionMediaType' + pane.key] || undefined}
                                                    ...
                                                    <Col span={5} offset={3}>
                                                        <FormItem label="Winning Criteria">
                                                            <Select getPopupContainer={triggerNode => triggerNode.parentNode}
                                                                ...
                                                                optionFilterProp="children"
                                                                value={this.state['optionAnswerType' + pane.key] || undefined}
                        ...
                        {this.state.category === "text" ? <div>
                            <Row>
                                <Col span={24} className="redsfun2winoptional">
                                    <Col span={6} lg={{ span: 13 }} xl={{ span: 12 }} className="Editredfuntext">
                                        <FormItem label="Type Your Question OR Brief(Optional)">
                                            <TextArea rows={3} placeholder="I Enter here" name={this.state.queueId + "@SubQuestion1"} onChange={this.handleInputChange} value={this.state[this.state.queueId + "@SubQuestion1"]} />
                                        </FormItem>
                                    ...
                                    <Col span={5}>
                                        <FormItem label="Add Answers">
                                            <Select getPopupContainer={triggerNode => triggerNode.parentNode}
                                                ...
                                                placeholder="Question Category"
                                                optionFilterProp="children"
                                                value={this.state.optionMediaTypeSubQuestion1 || undefined}
                                    ...
                                    <Col span={5} offset={3}>
                                        <FormItem label="Winning Criteria">
                                            <Select getPopupContainer={triggerNode => triggerNode.parentNode}
                                                ...
                                                placeholder="Opinion Based"
                                                optionFilterProp="children"
                                                value={this.state.optionAnswerTypeSubQuestion1 || undefined}
                                                ...
                                                {this.state.optionTextArraySubQuestion1.length > 0 ? this.state.optionTextArraySubQuestion1.map((imgs, index) => {
                                                    return (
                                                        <Col span={12}>
                                                            <FormItem className="redfinedfun2intxtoptns">
                                                                <div className="redotionlabel">
                                                                    <h3 className="SubTxtOptions"> <span> {"Option" + (index + 1)} </span>  <span className="Redfinedtxtdelicon"> <Icon type="close-square-o" className="paddingLftRht15 IconClose" onClick={this.deleteOptionContent.bind(this, imgs)} /></span></h3>
                                                                </div>
                                                ...
                                                {this.state.optionImageUploadArraySubQuestion1.length > 0 ? this.state.optionImageUploadArraySubQuestion1.map((imgs, index) => {
                                                    return (
                                                        <Col span={6} sm={{ span: 8 }} lg={{ span: 6 }} xl={{ span: 6 }} className="smallRefinedimagetypetopmainhead">
                                                            <FormItem>
                                                                <div className="Refinedimagetypetopmainhead">
                                                                    <h3 className="redfinedimagetypeptionslabel"><span>{"Option" + (index + 1)}</span><span className="redinedfun2winimgdel2"><Icon type="close-square-o" className="paddingLftRht15 IconClose" onClick={this.deleteOptionContent.bind(this, imgs)} /></span></h3>
                                                                </div>
                        ...
                        {this.state.panes.length > 0 ? <Col span={24}>
                            <FormItem label=" ">
                                <Button type="primary" className='createFun2Win' onClick={this.savePackQuestion.bind(this, this.state.queueId)}>Update</Button>
                            </FormItem>
                        </Col> : null}
                    </div>
                </div>
