...
class JunkLibrary extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            module: '',
    ...
    getMcContestTitles = (challengesType) => {
        var _this = this;
        const url = '/contests/curation/contestTitle/' + challengesType + '/junk' ;
      ...
      getChallengeType = (contestpagenumber,contestIds) => {
        ...
        const url = '/contests/curation/junk/' + contestpagenumber;
        var request = new Request(url, {
          method: 'POST',
    ...
    getfanclubFeeds = (feedtype, pagenumber) => {
        var _this = this;
        const url = '/fanclubs/feed/curation/' + feedtype + '/junk/' + pagenumber;
        var request = new Request(url, {
            method: 'GET',
            headers: {
                "Content-Type": "application/json",
    ...
    getfanclubComments = (pagenumber) => {
        var _this = this;
        const url = '/fanclubs/comments/curation/junk/' + pagenumber;
        var request = new Request(url, {
            method: 'GET',
            headers: {
                "Content-Type": "application/json",
    ...
    render() {
        ...
        for (let key in this.state.fanclubComments) {
            ...
            commentData.push(
                <div><Col span={12} xs={12} sm={12} xl={8} lg={12} md={12} >
                    <div className="fanClubCuratortextjunkfeed">
                        <ul className="list-inline">
                            <li>
        ...
        var container = (
            <div className="JunlLibrary-aalert">
            <Alert
        ...
        return (
            <Dashboard>
                <div className="ContentCuration-New">
                    <div>
                        <Row>
                            <Col span={24} className="HeaderNewContnetc">
                                <Col span={4}>
                                    <h3 className="ContentCurationNavmainHeader"><Link to="/Curation"><span><Icon type="arrow-left" /></span></Link> <span>Junk Library</span></h3>
                                </Col>
                    ...
                    <div className="JunkTotalFiles">
                        {(this.state.module === "Challenges" && this.state.challengestypes) ?
                            <Row className="CuratorRoeshjunk">{challengesData.length > 0 ? challengesData : <h1>{container}</h1>} </Row> :
                            (this.state.module === "FanclubFeeds" && this.state.feedtype)
                                ? <div><Row className="CuratorRoeshjunk"> {feedData.length > 0 ? feedData : <h1>{feedAlert}</h1>}</Row></div> :
