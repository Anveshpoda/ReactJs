...
import { Menu, Col, Row, Card, Button, Dropdown, Icon, message, Spin, Modal, Alert } from 'antd';
...
class RunningList extends React.Component {
  ...
  //   })
  //     .then(function (response) {
  //       if (response.data.message === 'Contest successfully deleted.') {
  //         con.getContests();
  //         message.success("contest is deleted successfully", 5);

  //       } else {
  //         message.error("Unable to delete the contest.", 5);
  //       }
  //     })
  //     .catch(function (error) {
  //       console.log(error);
    ...
    axios.get('/contest/' + id, {
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          var myObj = response.data.data.dubshmash
          if (isEmpty(myObj)) {
            $(".dub444").hide();
          } else {
            $(".dub444").show();
          }
          var myObj = response.data.data.aboutMovie
          if (isEmpty(myObj)) {
            $(".hidecrewcast").hide();
            $(".hidesyrele").hide();
          } else {
          ...
          var myObj = response.data.data.frame
          if (isEmpty(myObj)) {
            $(".selfiwithframe").hide();
          } else {
            $(".selfiwithframe").show();
  ...
  handleOk = (e) => {
    this.setState({
      visible: false, hookvisible: false, viewdata33: [], viewdata43: [], viewdata3: [], viewdata4: [], viewdata31: [], viewdata32: [], viewdata41: [], viewdata42: [], viewdata: {}, viewdata0: {}, viewdata1: {}, viewdata2: {}, modalloading: true
    });
  }
  handleCancel = (e) => {
    this.setState({
  ...
  showHookModal = (e) => {
    ...
    axios.get('/contest/' + id, {
      ...
      .then(function (response) {
        if (response.status === 200) {
          console.log("Imaage:-----", response.data.data);
          console.log("trmes", response.data.data.termsAndConditions);
          _this.setState(
            {
              hookImage: response.data.data.contestIcon,
              hookDescription: response.data.data.contestDescription,
              hookCaption: response.data.data.contestCaption,
              hookTitle: response.data.data.contestTitle
            });
          _this.setState({ value: RichTextEditor.createValueFromString(response.data.data.termsAndConditions, 'html') });
          var myObj = response.data.data.karoke
          if (isEmpty(myObj)) {
  ...
  handleHook = () => {
    ...
    console.log("hookImage------->", _this.state.hookImage);
    const url = 'https://graph.facebook.com/v2.11/1716641351947974/feed';
    var fbtoken = "EAAFYy26FH4MBAOkaZBKMlJDAALqgCJAHBEvvZBPpSbQK6E8fXCyJlzRGZCdek5G7zRHqbZAHdAZCI7veb95ZCTGuO9oZAkKwugou4XZB2mZBKQWiRAAEq0WDXcCMJOoyHmB6oqGrL9ZAitWy3qHoAAsBaTQcuktNCcrCj59ZAFIssiVUOamXuZCMNARrYiC7bCzMA6qNd9WA142xKQZDZD";
    $.ajax({
      url: "https://graph.facebook.com/1716641351947974/feed",
      type: "POST",
      data: {
        message: _this.state.hookTitle,
        link: "https://fankick.io/",
        source: "https://fankicklive.blob.core.windows.net/images/269201710.jpg",
        //source: _this.state.hookImage,
        access_token: fbtoken,
        description: _this.state.hookDescription,
        format: "json"
      },
      success: function (response) {
        console.log(response)
        message.success('Post Challenge closed successfully!');
      },
      error: function (err) {
        if (err.status == 400) {
          message.error(`unable to Post in facebook`);
        }
  ...
  render() {
        ...
        <Col span={6} xs={12} sm={12} xl={4} lg={6} md={6}>
          <Card>
            <div className="image hover01">
              <figure>
                <img id={contest._id} onClick={this.showModal} src={contest.contestIcon} alt="Contest Cover" width='100%' height='175px'
                  className="BordrStyle challengesmaincardImage" />
              </figure>
            </div>
            <div className="cardContent">
              <ul key={index}>
                ...
                <li className="CardFootercolor"><b>{/*:{contest.pCount}*/}</b></li>
            ...
            {/* <div>
              <ul className="list-inline messagecentcountul">
                <li className="MessageCentcount1">
                  <h4><span><Icon type="eye" theme="outlined" /></span><span>20</span></h4>
                </li>
                <li className="MessageCentcount1">
                  <h4><span><Icon type="heart" theme="outlined" /></span><span>20</span></h4>
                </li>
                <li className="MessageCentcount1">
                  <h4><span><Icon type="user-add" theme="outlined" /></span><span>20</span></h4>
                </li>
                <li className="MessageCentcount1">
                  <h4><span><Icon type="copyright" theme="outlined" /></span><span>20</span></h4>
                </li>
              </ul>
            </div>*/}
                  ...
                  <Dropdown getPopupContainer={triggerNode => triggerNode.parentNode} overlay={<Menu>
                   ...
                   <Link to={`/contest/${contest._id}`} style={{color:'rgba(0, 0, 0, 0.65)'}}>Edit</Link>
                    ...
                    {this.state.role.running.close === true ?
                      ...
                      : null}
                    {this.state.role.running.hook === true ?
                      <Menu.Item key="6">
                        <a id={contest._id} onClick={this.showHookModal}>Hook</a>
                      </Menu.Item>
                      : null}
                  </Menu>} trigger={['click']}>
    ...
    if (currentTodos.length === 0) {
      ...
      return (
        <div>
          <div className="placeholderImages paddingLeft15">
            <Col span={6}><img src={Placeholder} className="img-responsive" /></Col>
            <Col span={6}><img src={Placeholder} className="img-responsive" /></Col>
            <Col span={6}><img src={Placeholder} className="img-responsive" /></Col>
            <Col span={6}><img src={Placeholder} className="img-responsive" /></Col>
            <Col span={6}><img src={Placeholder} className="img-responsive" /></Col>
    ...
    <p><b>ContestCaption</b><br /> {this.state.viewdata.contestCaption}</p
                    ...
                    <Row>
                      <Col span={11} className="CViewdetails">
                        ...
                        <span >
                          {this.state.viewdata.order}</span>
            ...
            <Modal className="cmodalB"
              title="Hook Message Centere to social Media"
              visible={this.state.hookvisible}
              onOk={this.hookOk}
              onCancel={this.hookCancel}
              width='817'
              style={{ top: 20 }}
              footer={null}
              >
                  ...
                  <div className="challengeDetails">
                    <Row className="challnegemainimages">
                      <div className="Cimagesviewdetails">
                        ...
                        <Col span={22} style={{ align: 'center' }}>
                          <h3>Banner Image</h3>
                          <img src={this.state.viewdata.contestIcon} alt="Banner" width='100%' />
                        </Col>
              ...
              <Col className="modalFooter">
                <div className="textRight">
                  <Button type="primary" onClick={this.handleHook}> Hook </Button>
                </div>
              </Col>
            </Modal>
