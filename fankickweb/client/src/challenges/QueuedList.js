...
import { Menu, Col, Row, Card, Button, Dropdown, message, Icon, Spin, Modal, Popconfirm, Alert } from 'antd';
...
class QueuedList extends React.Component {
  ...
  getContests = () => {
    axios.get('/queued', {
      ...
      .then(function (response) {
        ...
        // for (let key in hhh) {
        //     var keyObj = hhh[key];
        //     if (new Date(keyObj.contestStartDate).getTime() > new Date().getTime()) {
  ...
  deleteUser = (e) => {
    ...
    sessionStorage.setItem('deleteuserinchallenges', e.target.id)
  }
  showModal = (e) => {
    var id = e.target.id;
  ...
  render() {
        ...
        <Col span={6} xs={12} sm={12} xl={4} lg={6} md={6}>
          <Card>
            <div className="image hover01">
              <figure>
                <img id={contest._id} onClick={this.showModal} src={contest.contestIcon} alt="Contest Cover" width='100%' height='175px' className="BordrStyle challengesmaincardImage"
                  />
              </figure>
            </div>
            ...
            <div>
              <ul className="list-inline messagecentcountul">
                ...
                <li className="ColorOrange"><b>Queued to Approval</b></li>
              </ul>
            </div>
                ...
                <li className="floatRight">
                  <Dropdown getPopupContainer={triggerNode => triggerNode.parentNode} overlay={<Menu>
                    {this.state.role.queued.approve === true ?
                      <Menu.Item key="0">
                        <a ><span id={contest._id} onClick={this.approveUser.bind(this)} >Approve</span></a>
                      </Menu.Item>
                      : null}
                    {this.state.role.queued.edit === true ?
                      <Menu.Item key="1">
                        <Link to={`/fankick/${contest._id}`} className="colorNearBlck">Edit</Link>
                      </Menu.Item>
                      : null}
    ...
    if (currentTodos.length === 0) {
      return (
        <div>
          <div className="paddingLeft15" className="placeholderImages">
