...
let uuid = 0;
var questions = [];
class EditFun extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: 1,
    ...
    this.getFun2Win = this.getFun2Win.bind(this);
  ...
  getFun2Win(category) {
    const id = this.props.params ? this.props.params._id : this.props.packId;
    var _this = this;
    var instance = axios.create({
      headers: { 'x-access-token': sessionStorage.getItem('token') }
    });
    instance.get('/pack/' + id).then((response) => {
      _this.setState({ editfunid: id });
      _this.setState({ editfun2win: response.data.data });
      _this.setState({ categoryId: _this.state.editfun2win.categoryId });
      _this.setState({ subCategoryId: _this.state.editfun2win.subCategoryId });
      _this.setState({ title: _this.state.editfun2win.name });
      _this.setState({ commentsCount: _this.state.editfun2win.commentsCount });
      _this.setState({ likesCount: _this.state.editfun2win.likesCount });
      _this.setState({ viewsCount: _this.state.editfun2win.viewsCount });
      _this.setState({ playedCount: _this.state.editfun2win.playedCount });
      _this.setState({ celebrity: _this.state.editfun2win.celebrityName });
      _this.setState({ description: _this.state.editfun2win.description });
      _this.setState({ fancoins: _this.state.editfun2win.points });
      _this.setState({ duration: _this.state.editfun2win.duration });
      _this.setState({ imageUrl: _this.state.editfun2win.imageURL });
      _this.setState({ brandAndroidUrl: _this.state.editfun2win.brandAndroidUrl });
      _this.setState({ brandIconUrl: _this.state.editfun2win.brandIconUrl });
      _this.setState({ brandIosUrl: _this.state.editfun2win.brandIosUrl });
      _this.setState({ brandName: _this.state.editfun2win.brandName });
      _this.setState({ brandText: _this.state.editfun2win.brandText });
      _this.setState({ brandWebUrl: _this.state.editfun2win.brandWebUrl });
      _this.setState({ isBranded: _this.state.editfun2win.isBranded });
      _this.setState({ contentPack: _this.state.editfun2win.contentPackQuestions });
      var arr = [];
      for (let i = 0; i < _this.state.contentPack.length; i++) {
        var x = _this.state.contentPack[i];
        x.numElems = i;
        x.id = i;
        x.editable = false;
      ...
      _this.setState({ contentPackQuestions: arr })
      _this.getCelebrities(_this.state.subCategoryId);
    });
  }
  ...
  bindSubCategories(value) {
    var a = this.state.categories.map((categorydata) => {
      if (value === categorydata._id) {
        this.setState({
  ...
  add = (generatedkey) => {
    ...
    i++;
    this.setState({
      divId: i, generatedkey: generatedkey
    })
        ...
        if (this.state.contentPackQuestions.length > 0) {
          ...
          if (message1 === false) {
            let contentPack = {
              ...
              'brandAndroidUrl': this.state.brandAndroidUrl,
              'brandIosUrl': this.state.brandIosUrl,
              'brandWebUrl': this.state.brandWebUrl,
              'brandIconUrl': this.state.brandIconUrl,
            }
    ...
    fetch(request)
      ...
      .then(function (response) {
        if (response.status === 200) {
          ...
          browserHistory.push("/fun2win/fun2Winpage");
          questions = [];
          _this.setState({ contentpacklength: 0 })
        }
        else {
  ...
  render() {
    console.log("this.state in edit fun2win", this.state)
    ...
    var ui = <div>
      <div>
        <Row className="Fun2winEditSubNav">
          ...
          <Col span={8} className="margincatgrybtn funeditbtnu">
            {this.props.params ? <Button type="primary" className='createFun2Win' onClick={() => this.warningModal()}> <Icon type="arrow-left" />Back to Fun2Win</Button> : ''}
          </Col>
        </Row>

        <Row>
          <Col span={24}>
            <div className="EditFun2WinBodymain">
                    ...
                    <Row>
                      <Col span={14} className="">
                        ...
                        <FormItem>
                          ...
                          <Input readOnly={read} placeholder="Enter contestTitle" autoComplete={'off'} maxLength={70} name="title" value={this.state.title} onChange={this.handleInputChange} />
                    ...
                    <Row>
                      <Col span={14} className="">
                        <FormItem>
                          ...
                          <TextArea readOnly={read} rows={3} value={this.state.description} maxLength={150} name="description" placeholder="Challenge Description" onChange={this.handleInputChange} />
                        </FormItem>
                      </Col>
                      <Col>
                        <span>
                    ...
                    <Row>
                      <Col span={3} className="">
                        <FormItem>
                          ...
                          <InputNumber readOnly={read} min={1} max={1000} placeholder="FanCoins Awarded" name="fancoins" value={this.state.fancoins} onChange={this.handleFanCoinsChange} />
                      ...
                      <Col span={3} className="marginLeft20">
                        <FormItem>
                          ...
                          <InputNumber readOnly={read} min={1} max={1000} placeholder="Duration" name="duration" value={this.state.duration} onChange={this.handleDurationChange} />
                      ...
                      <Col span={5} className="">
                        <FormItem>
                          ...
                          <Upload {...props}
                            ...
                            disabled={this.props.params ? false : true}
                ...
                <TabPane tab="ADD QUESTIONS" key="2">
                  ...
                  <Row>
                    <Form className="QuestionMainForm">
                      ...
                      <Row>
                        <Col span={5} className="btnRight">
                          <Button type="primary" className='createFun2Win' onClick={this.submitContentPack.bind(this)} disabled={this.state.disabled} >Edit Fun2 Win</Button>
                        </Col>
                      </Row>
                    </Form>
                  </Row>
...
const EditFun2Win = Form.create()(EditFun);
export default EditFun2Win;
/* eslint-disable */