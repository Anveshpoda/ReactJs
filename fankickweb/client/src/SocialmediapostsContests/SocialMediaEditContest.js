...
class SocialMediaEditContest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      current: 0,
    ...
    fetch(request)
      ...
      .then(function (response) {
        if (response.message == "Success" && response.status == 200) {
          ...
          _this.setState({
            BannercontestImageUrl: _this.state.viewData.contestUrl,
            BannercontestwithoutTextImageUrl: _this.state.viewData.fbContestThumbnail,
            socialmediatype: 'Facebook',
            startDate: moment(_this.state.viewData.contestStartDate).format('MM/DD/YYYY'),
            endDate: moment(_this.state.viewData.contestEndDate).format('MM/DD/YYYY'),
            termsandConditions: _this.state.viewData.termsAndConditions,
            contestCaption: _this.state.viewData.caption,
            contestTitle: _this.state.viewData.fbContestName,
            contestsType: _this.state.viewData.title,
            contestDescription: _this.state.viewData.description,
            isYoutubeVideo: _this.state.viewData.isYoutubeVideo,
            youtubevideoid: _this.state.viewData.fbContestVideoUrl,
            mediaType: _this.state.viewData.contestType,
            categoryId: _this.state.viewData.categoryId,
            subCategoryId: _this.state.viewData.subCategoryId,
            celName: _this.state.viewData.celebrityName,
            ctaButtonText: _this.state.viewData.buttonText,
            winnersMessage: _this.state.viewData.winningMessage,
            location: _this.state.viewData.location,
            bonusFancoins: _this.state.viewData.prizeCoins,
            winnersCount: _this.state.viewData.winnersCount,
            merchantName: _this.state.viewData.couponFrom,
            couponCost: _this.state.viewData.couponCost,
            userGuideText: _this.state.viewData.userguideText,
            basicFancoins: _this.state.viewData.basicCoins,
            coinsConfig: _this.state.viewData.coinsConfig
          });
          {
            _this.state.isYoutubeVideo === true && _this.state.viewData.contestType == "video" ? _this.setState({ mediaType: "youtubevideo" }) :
              _this.state.viewData.contestType == "image" ? _this.setState({ mediaType: "image" }) : _this.setState({ mediaType: "normalvideo" })
          }
          { _this.state.coinsConfig ? _this.setState({ basiccoinsview: true }) : _this.setState({ basiccoinsview: false }) }
          {
            _this.state.viewData.goodieType == 1 ? _this.setState({ goodieType: "Coupons" }) :
              _this.state.viewData.goodieType == 2 ? _this.setState({ goodieType: "Fancoins" }) :
                _this.state.viewData.goodieType == 3 ? _this.setState({ goodieType: "Fancoins & Coupons" }) : ''
          }
  ...
  editContest = () => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const url = '/EditSocialmediacontest/' + this.props.params.id;
      var request = new Request(url, {
        method: 'PUT',
      ...
      fetch(request)
        ...
        .then(function (response) {
          if (response.message == "Success" && response.status == 200) {
            message.success("Facebook contest Edited successfully");
  ...
  next() {
    ...
    if (current === 0) {
      ...
      if (this.state.userGuideText.trim() === '') errors.userGuideText = "* This field is mandatory";
      ...
      content: <Col span={24}>
        <Form>
            ...
            <Col span={6} xl={{ span: 6 }} lg={{ span: 7 }} sm={{ span: 8 }} className="MrgnLeft701">
              <FormItem label="Contest Type">
                ...
                <Select getPopupContainer={triggerNode => triggerNode.parentNode}
                  ...
                  {/* <Option value="Share & Win">Share & Win</Option>
              ...
              <Col span={10} xl={{ span: 10 }} lg={{ span: 11 }} sm={{ span: 12 }}>
                <FormItem label="Contest Title">
                  <Input type="text" value={this.state.contestTitle}
                    onChange={this.contestTitleChange.bind(this)} autoComplete={'off'} placeholder="Enter Contest Title" name="" />
          ...
          <Col span={24}>
            <Col span={24} className="">
              <Col span={10} xl={{ span: 10 }} lg={{ span: 11 }} sm={{ span: 12 }}>
                <FormItem label="Contest Caption">
                  <Input
            ...
            <Col span={24} className="">
              <Col span={10} xl={{ span: 10 }} lg={{ span: 11 }} sm={{ span: 12 }}>
                <FormItem label="User Guiding Text">
                  <TextArea
              ...
              <Col span={3} xl={{ span: 5 }} lg={{ span: 5 }} sm={{ span: 5 }}>
                <FormItem label="Start Date">
                  ...
                  <DatePicker onChange={this.startDateChange.bind(this)}
                    value={moment(this.state.startDate, dateFormat)} getCalendarContainer={triggerNode => triggerNode.parentNode}
                    format={dateFormat} />
              ...
              <Col span={3} xl={{ span: 5 }} lg={{ span: 5 }} sm={{ span: 5 }} offset={1} className="MrgnLeft702">
                <FormItem label="End Date">
                  <DatePicker onChange={this.endDateChange.bind(this)}
                    value={moment(this.state.endDate, dateFormat)} getCalendarContainer={triggerNode => triggerNode.parentNode}
                    format={dateFormat} />
              ...
              <Col span={6} xl={{ span: 6 }} lg={{ span: 9 }} sm={{ span: 11 }} className="">
                <FormItem>
                  <ul className="list-inline socialmElists">
                    ...
                    <li>
                      <Switch checked={this.state.coinsConfig} onChange={this.coinsConfigChange.bind(this)} /> <br />
                    </li>
                  </ul>
            ...
            <div className="SocialMediamenu">
              <div className="SocialMediaContestSubMenu">
                <Col span={2} xl={{ span: 2 }} lg={{ span: 2 }} sm={{ span: 3 }} className="fbcontresttotitle">
                  <h2 className="smcontestpageTitle">Edit SM Contest</h2>
                </Col>
              ...
              <Col span={16} xl={{ span: 16 }} lg={{ span: 18 }} sm={{ span: 20 }} className="Createsocialmcontests">
                <div className="CreateData2">
                  <Col span={24} className="CreateCrudSelect">
                    <div className="">
                      <h2 className="BorderBottomGrey">Edit Contest Details</h2>
                      <div className="StepsMargn25">
                        ...
                        <div className="steps-action  floatRight">
                          ...
                          {
                            current === steps.length - 1
                            && <Button type="primary" onClick={this.editContest.bind(this)}>Edit Contest</Button>
                          }
...
export default SocialMediaEditContest;
/* eslint-disable */