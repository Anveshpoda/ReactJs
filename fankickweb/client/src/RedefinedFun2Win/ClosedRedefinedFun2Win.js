...
import { Col, Row, Icon, Button, Modal, Dropdown, Menu, Card, Select, Pagination } from 'antd';
...
class fun2WinRedefinedpage extends React.Component {
  ...
  componentWillMount() {
    ...
    if (name === 'Freelancer') {
      ...
      let url = '/content-packs/closed/' + loginId + '/1'
      this.getAllContentPacks(url)
    } else {
      this.setState({
        loginId: 'notFreelancer'
      })
      let url = '/content-packs/closed/notFreelancer/1'
      this.getAllContentPacks(url)
    }
    this.getcategories()
  }
  ...
  bindSubCategories(category) {
    this.state.categories.map((categorydata) => {
      if (category === categorydata._id) {
        ...
        let url = '/packs-by-subcategory/Closed/1/' + this.state.loginId + '?subCatId=' + categorydata.subCategories[0]._id
        this.getAllContentPacks(url)
      }
    })
  }
  ...
  onSubCategoryChange = (e) => {
    ...
    let url = '/packs-by-subcategory/Closed/1/' + this.state.loginId + '?subCatId=' + e
    this.getAllContentPacks(url)
  ...
  onCelbrityChange = (e) => {
    ...
    let url = '/packs-by-celebrityname/Closed/1/' + this.state.loginId + '?celName=' + e
    this.getAllContentPacks(url)
  }
  ...
  onPaginationChange = (page, pageSize) => {
    ...
    if ((this.state.subCategoryId !== '') && (this.state.celebrity === '')) {
      url = '/packs-by-subcategory/Closed/' + page + '/' + this.state.loginId + '?subCatId=' + this.state.subCategoryId;
    }
    else if (this.state.celebrity !== '') {
      url = '/packs-by-celebrityname/Closed/' + page + '/' + this.state.loginId + '?celName=' + this.state.celebrity;
    }
    else {
      url = '/content-packs/closed/' + this.state.loginId + '/' + page
    }
    axios.get(url, {
      headers: {
              ...
              return (<Col span={6} xs={12} sm={12} xl={4} lg={6} md={6}>     <Card className="margintop20">
                ...
                <div className="cardContent">
                  ...
                  <ul className="fun2winiconsstatuscount">
                    {element.isDeleted ? <b style={{ color: "orange" }}>Closed</b> : null}
                  </ul>
                </div>
