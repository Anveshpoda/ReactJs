...
import { Col, Row, Icon, Button, Modal, Dropdown, Menu, Card, Select, Pagination, message } from 'antd';
...
class fun2WinRedefinedpage extends React.Component {
  ...
  componentWillMount() {
    ...
    if (name === 'Freelancer') {
      ...
      let url = '/content-packs/queued/' + loginId + '/1'
      this.getAllContentPacks(url)
    } else {
      this.setState({
        loginId: 'notFreelancer'
      })
      let url = '/content-packs/queued/notFreelancer/1'
      this.getAllContentPacks(url)
    }
    this.getcategories()
  }
  ...
  bindSubCategories(category) {
    this.state.categories.map((categorydata) => {
      if (category === categorydata._id) {
        ...
        let url = '/packs-by-subcategory/Queued/1/' + this.state.loginId + '?subCatId=' + categorydata.subCategories[0]._id
        this.getAllContentPacks(url)
      }
    })
  }
  ...
  onSubCategoryChange = (e) => {
    ...
    let url = '/packs-by-subcategory/Queued/1/' + this.state.loginId + '?subCatId=' + e
    this.getAllContentPacks(url)
  ...
  onCelbrityChange = (e) => {
    ...
    let url = '/packs-by-celebrityname/Queued/1/' + this.state.loginId + '?celName=' + e
    this.getAllContentPacks(url)
  }
  ...
  onPaginationChange = (page, pageSize) => {
    ...
    if ((this.state.subCategoryId !== '') && (this.state.celebrity === '')) {
      url = '/packs-by-subcategory/Queued/' + page + '/' + this.state.loginId + '?subCatId=' + this.state.subCategoryId;
    }
    else if (this.state.celebrity !== '') {
      url = '/packs-by-celebrityname/Queued/' + page + '/' + this.state.loginId + '?celName=' + this.state.celebrity;
    }
    else {
      url = '/content-packs/queued/' + this.state.loginId + '/' + page
    }
    axios.get(url, {
      headers: {
            ...
            <Col span={24} className='challengesmenu'>
              <div className="fun2winSubMenu">
                ...
                <Col span={3} sm={{ span: 4 }} lg={{ span: 3 }} xl={{ span: 3 }} className="redfinedfundashselects redefineddshfirstselec">
                  <Select className="redfinedrunningselectors"
                ...
                <Col span={4} sm={{ span: 4 }} lg={{ span: 4 }} xl={{ span: 3 }}>
                  <Select className="redfinedrunningselectors"
                    showSearch getPopupContainer={triggerNode => triggerNode.parentNode}
                    style={{ width: '100%' }}
                ...
                <Col span={9} className="fun2winmainheader redefinfunmainsubheader">
                  <nav className='challengesnavigation fun2winNavigations' role="navigation">
                    ...
                    <Link to="/fun2WinRedefinedpage/createFun2WinNewPage"> <Button type="primary" className='createFun2WinBtn'>Create Fun2Win</Button></Link>
                  </nav>
                </Col>
              </div>
            </Col>
            ...
            {this.state.contentpacks.map((element) => {
              return (<Col span={6} xs={12} sm={12} xl={4} lg={6} md={6}>     <Card className="margintop20">
                <div className="image hover01">
                  <figure>
                    <img className="fun2winbannerimage fun2WinMainCardImages" id={element._id} onClick={this.showModal} src={element.imageURL} alt={element.imageURL}
                ...
                <div className="cardContent">
                  ...
                  <ul className="fun2winiconsstatuscount">
                    {!element.isPublished && !element.isDeleted ? <b style={{ color: "orange" }}>Queued to Approval</b> : null}
                  </ul>
                </div>
                <div className="cardFooter">
                  <ul className="list-inline">
                    ...
                    <li className="Fun2winiconmain">
                      <img src={participantscount12} alt="contest-cover" className="fun2winpartcount" />  <span>0 </span>
                    </li>
                    <li className="Fun2winiconmain">
                      <ul className="list-inline">
                        <li>
