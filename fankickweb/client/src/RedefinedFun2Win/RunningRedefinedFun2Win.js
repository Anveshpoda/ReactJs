...
class fun2WinRedefinedpage extends React.Component {
  ...
  componentWillMount() {
    ...
    if (name === 'Freelancer') {
      ...
      let url = '/content-packs/live/' + loginId + '/1'
      this.getAllContentPacks(url)
    } else {
      this.setState({
        loginId: 'notFreelancer'
      })
      let url = '/content-packs/live/notFreelancer/1'
      this.getAllContentPacks(url)
    }
    this.getcategories()
  }
  ...
  bindSubCategories(category) {
    this.state.categories.map((categorydata) => {
      if (category === categorydata._id) {
        ...
        let url = '/packs-by-subcategory/Live/1/' + this.state.loginId + '?subCatId=' + categorydata.subCategories[0]._id
        this.getAllContentPacks(url)
      }
    })
  }
  ...
  onSubCategoryChange = (e) => {
    ...
    let url = '/packs-by-subcategory/Live/1/' + this.state.loginId + '?subCatId=' + e
    this.getAllContentPacks(url)
  ...
  onCelbrityChange = (e) => {
    ...
    let url = '/packs-by-celebrityname/Live/1/' + this.state.loginId + '?celName=' + e
    this.getAllContentPacks(url)
  }
  ...
  onPaginationChange = (page, pageSize) => {
    ...
    if ((this.state.subCategoryId !== '') && (this.state.celebrity === '')) {
      url = '/packs-by-subcategory/Live/' + page + '/' + this.state.loginId + '?subCatId=' + this.state.subCategoryId;
    }
    else if (this.state.celebrity !== '') {
      url = '/packs-by-celebrityname/Live/' + page + '/' + this.state.loginId + '?celName=' + this.state.celebrity;
    }
    else {
      url = '/content-packs/live/' + this.state.loginId + '/' + page
    }
    axios.get(url, {
      headers: {
                ...
                <Card className="margintop20">
                  ...
                  <div className="cardContent">
                    ...
                    <ul className="fun2winiconsstatuscount">
                      {element.isPublished && !element.isDeleted ? <b style={{ color: '#783293' }}> {/* Participants :<span>{element.answeredCount}</span>*/}</b> : null}
                    </ul>
                  </div>
